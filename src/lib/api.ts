...
class MondeAPI {
  ...
  private async request<T>(
    ...
    if (!response.ok) {
      ...
      // Se for erro 401, limpar token inválido e disparar evento
      if (response.status === 401) {
        this.logout();
        console.warn('Token inválido removido. Redirecionando para login.');
        // Disparar evento customizado para o app saber que o token expirou
  ...
  async getTaskCategories(): Promise<ApiResponse<TaskCategory[]>> {
    // Sort by description per API docs (name is invalid)
    return this.request<ApiResponse<TaskCategory[]>>("/task-categories?page[size]=50&sort=description");
  }
