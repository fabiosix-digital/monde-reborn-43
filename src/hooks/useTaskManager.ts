...
export function useTaskManager(options: UseTaskManagerOptions = {}) {
  ...
  const updateTaskStatus = useCallback(async (taskId: string, completed: boolean, newDueDate?: string) => {
    try {
      console.log("Updating task status:", { taskId, completed, newDueDate });
