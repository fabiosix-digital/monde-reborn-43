...
export function TaskDetailsModal({
  ...
  const { data: relatedData, isLoading: loadingRelations } = useTaskRelationshipsOptimized(task?.id || null);
  
  // Buscar históricos da tarefa
  const { data: taskHistorics = [], isLoading: loadingHistorics } = useTaskHistorics(task?.id || null);
  
  // Buscar pessoas para seleção
  const { data: peoplePages } = usePeopleInfinite({ search: "" });

  // Buscar usuários da empresa (apenas usuários do sistema)
  const { data: companyUsersResponse } = useQuery({
  ...
  const handleReopenTask = async () => {
    ...
    try {
      const updateData: UpdateTaskData = {
        ...
        attributes: {
          completed: false,
          due: format(addHours(new Date(), 24), 'yyyy-MM-dd\'T\'HH:mm:ss')
        }
      };
      ...
      await updateTaskMutation.mutateAsync({
        ...
        // Criar histórico da reabertura - Using 'historic' field for system changes
        try {
          const userName = (await api.getUserFromToken().catch(() => null))?.data?.attributes?.name || "Usuário";
          await api.createTaskHistoric({
            type: "task-historics",
            attributes: {
              historic: "Concluída alterado de 'Sim' para 'Não'",
              description: `${userName} - ${format(new Date(), "dd/MM/yyyy HH:mm", { locale: ptBR })}`,
          ...
          console.warn("Failed to create task historic for reopen:", historicError);
          // Continue execution since task was already reopened successfully
        }
    ...
    try {
      ...
      if (updateText.trim()) {
        const historicData = {
          ...
          attributes: {
            text: updateText.trim(), // Use 'text' field for user textarea input (manual updates)
            "date-time": new Date().toISOString()
          },
          relationships: {
            task: {
      ...
      if (nextReturnDate) {
        const dueDateTime = nextReturnTime 
          ? `${nextReturnDate}T${nextReturnTime}:00`
          : `${nextReturnDate}T09:00:00`;
        ...
        try {
          await api.createTaskHistoric({
            ...
            attributes: {
              ...
              description: `${currentUserName} - ${format(new Date(), "dd/MM/yyyy HH:mm", { locale: ptBR })}`,
              "date-time": new Date().toISOString()
            },
            relationships: {
              task: {
      ...
      queryClient.removeQueries({ queryKey: ["task-historics", task.id] });
      queryClient.removeQueries({ queryKey: ["task-historics"] }); // Remove all historics cache
      await queryClient.invalidateQueries({ queryKey: ["task-historics", task.id] });
      await queryClient.invalidateQueries({ queryKey: ["task-historics"] });
      
      // Aguardar um pouco e buscar novamente para garantir sincronização
      setTimeout(async () => {
        await queryClient.refetchQueries({ queryKey: ["task-historics", task.id] });
      ...
      toast({ title: "Atualização adicionada com sucesso!" });
    } catch (error) {
  ...
  const handleSaveEdits = async () => {
    ...
    try {
      ...
      // Invalidar cache
      queryClient.invalidateQueries({ queryKey: ["task-related-data", task.id] });
        ...
        <DialogHeader className="px-4 py-1 border-b bg-background">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              {!isEditing ? (
                <>
                  {task.attributes.completed || task.attributes.deleted ? (
                    <Button 
                      variant="outline" 
                      size="sm" 
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                {isEditing && !task.attributes.completed && !task.attributes.deleted ? (
                  <div className="mt-1 flex items-center gap-1">
                    <Select value={selectedPersonId} onValueChange={setSelectedPersonId}>
                      ...
                      <SelectContent>
                        ...
                        {peoplePages?.pages.flatMap(page => page.data).map((person) => (
                    ...
                    {selectedPersonId && selectedPersonId !== "none" && (
                      <Button 
                        ...
                        onClick={() => setSelectedPersonId("none")}
                      >
                        <X className="h-3 w-3" />
                      </Button>
                    )}
                  ...
                  <div className="mt-1 flex items-center gap-1">
                    ...
                    {selectedAssigneeId && selectedAssigneeId !== "none" && (
                      <Button 
                        ...
                        size="sm" 
                        className="h-6 w-6 p-0"
                        onClick={() => setSelectedAssigneeId("none")}
                      >
                        <X className="h-3 w-3" />
                      </Button>
                    )}
                ...
                {isEditing && !task.attributes.completed && !task.attributes.deleted ? (
                  <div className="mt-1">
                    <Input 
                      ...
                      className="h-8 text-sm bg-[#f1f5f9]"
                    />
                  </div>
                ) : (
                  <div className="mt-1 text-sm text-foreground">
            ...
            <div>
              ...
              <div className="mt-2">
                <Textarea 
                  ...
                  disabled={task.attributes.completed || task.attributes.deleted}
                  className="min-h-[60px] resize-none bg-[#f1f5f9] border-slate-200 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                />
              </div>
            </div>
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <Input 
                      ...
                      placeholder="-/-/-"
                      disabled={task.attributes.completed || task.attributes.deleted}
                      className="h-8 text-sm bg-[#f1f5f9] border-slate-200 disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                  </div>
                </div>
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    <Input 
                      type="time"
                      value={nextReturnTime}
                      onChange={(e) => setNextReturnTime(e.target.value)}
                      disabled={task.attributes.completed || task.attributes.deleted}
                      className="flex-1 h-8 text-sm bg-[#f1f5f9] border-slate-200 disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                    <Button 
                      size="sm" 
                      className="h-8 px-2"
                      onClick={handleAddUpdate}
                      disabled={updateTaskMutation.isPending || task.attributes.completed || task.attributes.deleted}
                    >
            ...
            <div className="space-y-2">
              ...
              {loadingHistorics ? (
                <div className="text-sm text-muted-foreground">Carregando históricos...</div>
              ) : taskHistorics.length > 0 ? (
                taskHistorics.map((historic, index) => {
                  // Alternar cores de fundo para diferentes históricos
                  const bgColor = index % 2 === 0 ? '#fef3c7' : '#f1f5f9';
                  
                  return (
                    <div 
                      key={historic.id || index} 
                      ...
                      <div className="flex-1 space-y-1">
                        <div className="flex items-center gap-2 text-xs">
                          ...
                          <span className="text-muted-foreground">
                            - {historic.attributes['date-time'] || historic.attributes['created-at'] 
                               ? formatDate(historic.attributes['date-time'] || historic.attributes['created-at']) 
                               : 'Data não disponível'}
                          </span>
                        </div>
