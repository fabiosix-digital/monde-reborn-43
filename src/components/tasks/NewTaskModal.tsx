    ...
    try {
      ...
      const data = mode === "create" 
        ? {
            ...
            relationships: {
              ...(editData.personId && {
                person: { data: { type: "people", id: editData.personId } }
              }),
              // Sempre incluir assignee e category para criação (obrigatórios)
              assignee: { data: { type: "users", id: editData.assigneeId } },
              category: { data: { type: "task-categories", id: editData.categoryId } }
            }
          }
        : {
            type: "tasks" as const,
        ...
        <div className="flex-1 overflow-auto p-4">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <div className="mt-1 p-3 bg-muted/50 rounded-md min-h-[120px]">
                    <p className="text-sm text-muted-foreground">
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      <Input
                        ...
                        value={editData.due}
                        onChange={(e) =>
                          setEditData((prev) => ({ ...prev, due: e.target.value }))
                        }
                        className="mt-1"
                      />
                    ) : (
                      <div className="mt-1 flex items-center gap-2 text-sm">
                        ...
                        <span>
                          {task?.attributes.due 
                            ? formatDate(task.attributes.due)
                            : "Não definido"
                          }
                        </span>
                      </div>
            ...
            <div className="space-y-6">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      ...
                      <div className="mt-1 text-sm">
                        {editData.personId 
                          ? people.find(p => p.id === editData.personId)?.attributes.name || "Cliente desconhecido"
                          : "Nenhum cliente"
                        }
                      </div>
                    )}
                    ...
                    {isEditing ? (
                      ...
                      >
                        ...
                        <SelectContent>
                          ...
                          {companyUsers.filter(user => user.id && user.id.trim() !== "").map((user) => (
                      ...
                      <div className="mt-1 text-sm">
                        {task?.relationships?.assignee?.data 
                          ? companyUsers.find(u => u.id === task.relationships.assignee.data.id)?.attributes.name || "Usuário desconhecido"
                          : "Nenhum responsável"
                        }
                      </div>
                    )}
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      ...
                      <div className="mt-1 text-sm">
                        {task?.relationships?.category?.data 
                          ? categories.find(c => c.id === task.relationships.category.data.id)?.attributes.name || "Categoria desconhecida"
                          : "Nenhuma categoria"
                        }
                      </div>
                    )}
              ...
              {task && (
                <TaskHistoricDisplay taskId={task.id} />
              )}
            </div>
          </div>
        </div>
